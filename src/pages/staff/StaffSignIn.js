// frontend/src/pages/staff/StaffSignIn.js

import React, { useState } from 'react';
import axios from 'axios';
import { useNavigate } from 'react-router-dom';
import './css/StaffSignIn.css';
import config from '../../config';

const StaffSignIn = ({ onClose }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post(`${config.apiUrl}/staff/staff-login`,
        {
          email,
          pwd: password,
        },
      ...
      if (response.data.token) {
        ...
        setEmail('');
        setPassword('');
        // Navigate to Staff.js after a successful login
        navigate('/staff');
      }
    } catch (err) {
